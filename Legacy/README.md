...
This README is current through the latest reconstructed project history in
`CHANGELOG.md` as of 2026-05-27, including merged PR #48. The active product
shape is a Rust-first, local-only investigation CLI with ticket-folder outputs,
offline fixture coverage, inbox follow-up chat, and Codex app-server support for
lower-latency inbox follow-ups. `unleash` remains the production LLM path;
`codex` is the dev/fallback path. Structured triage calls still use the safe
Codex `exec` path until app-server structured-output parity is explicitly
validated.
...
fixture directory and skips live Zendesk plus Datadog calls.
...
Agent definitions live in `agents/`. Edit those Markdown files to tune task scope, personality, source priority, stop conditions, and notification behavior without changing Rust code. Ticket-specific learning should go to `MEMORY.md` and `Tickets/customers/<slug>/HISTORY.md`; only repeated workflow lessons should become proposed edits to `agents/*.md`.
...
triage-cli/
├── README.md                       # this file
├── CHANGELOG.md                    # onboarding changelog and PR history
...
├── scratch/                        # transient workspace under TRIAGE_HOME
├── data/
...
└── triage-cli-rs/                  # Rust crate
    ...
    └── src/
        ...
        ├── pipeline/               # phase-based investigate/follow-up pipeline
        ...
        ├── providers/              # mod, unleash, codex, codex_app_server, codex_schema
...
cargo test --test integration   # offline runbook + CLI smoke coverage
cargo clippy --all-targets -- -D warnings
cargo fmt --all -- --check
```

Credentialed/live gates are opt-in:
...
See `triage-cli-rs/tests/README.md` for the full test-tier matrix.
