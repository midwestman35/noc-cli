...
def test_config_set_writes_env_file(tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        app, ["config", "set", "watch_assignee", "alice@carbyne.com"]
    )
...
def test_config_set_zendesk_api_token_masks_output_but_writes_raw_value(
    ...
    result = runner.invoke(
        app, ["config", "set", "zendesk_api_token", "raw-secret-token"]
    )
...
def test_setup_command_writes_env_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "noc_cli.cli.shutil.which", lambda name: "/usr/local/bin/claude"
    )
...
def test_doctor_exits_0_when_all_critical_checks_pass(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "noc_cli.doctor.shutil.which", lambda name: "/usr/local/bin/fake"
    )
